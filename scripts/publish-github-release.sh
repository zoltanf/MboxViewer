...
TAG="v${MBOX_VIEWER_BUILD_VERSION}"
TITLE="Mbox Viewer ${MBOX_VIEWER_BUILD_VERSION}"

if ! gh release view "$TAG" --repo "$SOURCE_REPO" >/dev/null 2>&1; then
  gh release create "$TAG" \
    --repo "$SOURCE_REPO" \
    --title "$TITLE" \
    --notes "Automated Mbox Viewer release for ${MBOX_VIEWER_BUILD_VERSION}."
fi
...
for file in "${ARTIFACT_FILES[@]}"; do
  ...
  if [[ ${#UPLOADS[@]} -eq 0 ]]; then
    UPLOADS+=("$MBOX_VIEWER_SHA256SUMS_PATH")
  fi
  UPLOADS+=(
    "$MBOX_VIEWER_DMG_PATH"
    "$MBOX_VIEWER_ZIP_PATH"
