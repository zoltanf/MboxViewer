...
TAP_REPO="${MBOX_VIEWER_TAP_REPO:-${SOURCE_OWNER}/homebrew-mboxviewer}"
...
if gh repo view "$TAP_REPO" >/dev/null 2>&1; then
  ...
  gh repo create "$TAP_REPO" --public --description "Homebrew tap for Mbox Viewer" >/dev/null
  gh repo clone "$TAP_REPO" "$TAP_DIR"
fi

node "$ROOT/scripts/render_homebrew_tap.js" \
  --tap-dir "$TAP_DIR" \
...
VERSION_LINE="$(sed -n "s/^MBOX_VIEWER_BUILD_VERSION='\(.*\)'$/\1/p" "${ARTIFACT_FILES[0]}")"
git -C "$TAP_DIR" commit -m "Update Mbox Viewer Homebrew tap to ${VERSION_LINE}"
git -C "$TAP_DIR" push
